...
class Schema {
  constructor(logger, executor, configMain) {
    ...
    this.createSchema = function (pool, callback) {
      _this.logger.debug('Schema', null, [`Creating schema if not exists: ${pool}`], false);
    ...
    this.createLocalShares = async function (pool) {
      _this.logger.debug('Schema', null, [`Creating local_shares table in schema: ${pool}`], false);
      const tableCommand = `
    ...
    this.handleDeployment = async function (pool) {
      ...
      await _this.handlePromises(pool, _this.selectLocalTransactions, _this.createLocalTransactions);
    };
